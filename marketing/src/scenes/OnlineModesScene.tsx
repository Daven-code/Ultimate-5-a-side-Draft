/******************************************************************************
 * OnlineModesScene.tsx - Landscape
 * =============================================================================
...
const ModeCard: React.FC<{mode: OnlineMode; index: number}> = ({mode, index}) => {
  ...
  return (
    ...
    >
      <div style={{color: mode.accent, fontFamily: "Bebas Neue", fontWeight: 950, fontSize: 34, letterSpacing: 2.4}}>
        MODE {index + 1}
      </div>
      <div style={{color: "white", fontFamily: "Bebas Neue", fontWeight: 950, fontSize: 54, lineHeight: 1.02, textTransform: "uppercase", textShadow: `0 0 18px ${mode.accent}88`}}>
        {mode.title}
      </div>
    </div>
  );
...
export const OnlineModesScene: React.FC<Props> = ({title, modes}) => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: "#020617", overflow: "hidden"}}>
      ...
      <LightRays />
      <Particles />
      <AbsoluteFill style={{alignItems: "center", paddingTop: 105, textAlign: "center"}}>
        <div style={{opacity: titleOpacity, color: "#FFFFFF", fontFamily: "Bebas Neue", fontWeight: 950, fontSize: 70, letterSpacing: 4, textTransform: "uppercase", textShadow: "0 0 28px rgba(96,165,250,0.7)"}}>
          {title}
        </div>
      </AbsoluteFill>
      <AbsoluteFill style={{alignItems: "center", justifyContent: "center", flexDirection: "row", gap: 38, paddingTop: 95}}>
