 ...
 * =============================================================================
 * Neymar-ready layout:
 * - Uses a dynamic row count so longer careers do not get clipped.
 * - Compresses card height / vertical spacing for 17+ clue careers.
 * - Keeps the nationality reveal clear of the final clue row.
 ******************************************************************************/
...
const ClueCard: React.FC<{clue: PlayerClue; index: number; framesPerClue: number;}> = ({clue, index, framesPerClue}) => {
  ...
  return (
    <div style={{width: cardWidth, height: cardHeight, opacity, transform: `translate(-50%, -50%) scale(${scale})`, borderRadius: 22, padding: "10px 16px", background: "linear-gradient(145deg, rgba(15,23,42,0.96), rgba(30,41,59,0.82))", border: "3px solid rgba(96,165,250,0.86)", boxShadow: "0 0 28px rgba(96,165,250,0.44), inset 0 0 18px rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", color: "white", fontFamily: "Bebas Neue", textAlign: "center", overflow: "hidden"}}>
      <div style={{fontSize: 26, color: "#60A5FA", letterSpacing: 2, lineHeight: 1}}>{clue.year}</div>
      <div style={{flex: 1, width: "100%", display: "grid", gridTemplateColumns: "1.05fr 0.95fr", alignItems: "center", gap: 10, paddingTop: 3}}>
        <div style={{display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "transparent"}}>
          {clue.crest ? <Img src={staticFile(clue.crest)} style={crestStyle} /> : <div style={{fontSize: clue.team.length > 14 ? 21 : 27, lineHeight: 0.93, letterSpacing: 1.1, textTransform: "uppercase", textShadow: "0 0 14px rgba(255,255,255,0.32)"}}>{clue.team}</div>}
        </div>
        <div style={{fontSize: 46, color: "#FDE68A", lineHeight: 1, textShadow: "0 0 14px rgba(250,204,21,0.55)"}}>{clue.rating}</div>
      </div>
    </div>
  );
};
...
const ConnectorArrows: React.FC<{careerLength: number; framesPerClue: number;}> = ({careerLength, framesPerClue}) => {
  ...
  return (
    <svg width={layoutWidth} height={layoutHeight} viewBox={`0 0 ${layoutWidth} ${layoutHeight}`} style={{position: "absolute", inset: 0, overflow: "visible", pointerEvents: "none"}}>
      ...
      {connectors.map((connector, index) => <path key={`connector-${index}`} d={connector.path} fill="none" stroke="#60A5FA" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" markerEnd="url(#arrowheadLandscape)" opacity={connector.opacity} style={{filter: "drop-shadow(0 0 5px rgba(96,165,250,0.85))"}} />)}
    </svg>
  );
};
...
export const GuessPlayerCluesScene: React.FC<Props> = ({career, nationalityLabel, nationality, nationalityFlag, framesPerClue}) => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: "#020617", overflow: "hidden"}}>
      ...
      <AbsoluteFill style={{alignItems: "center", paddingTop: 34, textAlign: "center", opacity: sceneEndOpacity}}>
      ...
      <div style={{position: "absolute", left: "50%", top: "50.2%", width: layoutWidth, height: layoutHeight, transform: "translate(-50%, -50%)", opacity: sceneEndOpacity}}>
